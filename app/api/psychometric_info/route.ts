...
export async function POST(req: NextRequest) {
  try {
    ...
    const items: Array<{ customerId?: string }> = Array.isArray(body)
      ? body
      : Array.isArray(body?.assessments) ? body.assessments : [];
    ...
    const backendRes = await cpClient.post('/psychometric_info/', body);

    customerIds.forEach((id) => {
      setCustomerCache(id, 'psychometric_info', body);
    });

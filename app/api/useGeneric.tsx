...
const defaultDataExtractor =  (response: any) => {
...
export const useGenericMethod = ({
  ...
  const handleAction = async (...args: any[]) => {
    ...
    try {
      ...
      if (method === 'GET' && resolvedResponse?.status === 200) {
        if (mountedRef.current){
          onSuccessPre?.(
            defaultDataExtractor(resolvedResponse)
          );
        }
  ...
  return { data, loading, error, handleAction, reset, refetch, setError, resetData };
};